class ZZMDG_CL_CS_PIR_IDOC_DRF_NEW definition
  public
  create public .
...
public section.
  ...
  constants GC_PIR_KEY_TAB type STRING value 'ZZMDG_T_DRF_PIR_KF' ##NO_TEXT.
...
private section.
  ...
  class-data GR_DRF_PIR type ref to ZZMDG_CL_CS_PIR_IDOC_DRF_NEW .
  data MS_PIR_KEY_DATA type ZZMDG_S_DRF_PIR_KF .
...
CLASS ZZMDG_CL_CS_PIR_IDOC_DRF_NEW IMPLEMENTATION.
  ...
  METHOD IF_DRF_OUTBOUND~ANALYZE_CHANGES_BY_MDG_CP.
    ...
    DATA: ls_change_pointer TYPE mdg_cp_s_cp_full,
          ls_pir_key        TYPE zzmdg_s_drf_pir_kf,
...
** Object keys are fetched from the change pointer
    LOOP AT lt_change_pointer INTO ls_change_pointer.
      ls_pir_key-infnr = substring( val = ls_change_pointer-object_key off = 0 len = 40 ).
...
*    gv_repl_mode = is_runtime_param-dlmod.
  ...
  METHOD IF_DRF_OUTBOUND~MAP_DATA2MESSAGE.
    ...
    ASSIGN COMPONENT 'infnr' OF STRUCTURE <ls_data> TO FIELD-SYMBOL(<lv_infnr>).


    IF <ls_data> IS ASSIGNED.
      ms_pir_key_data-infnr = <lv_infnr>.
    ENDIF.
  ...
  METHOD IF_DRF_OUTBOUND~SEND_MESSAGE.
    ...
    DATA :
      ...
      ls_pir_data            TYPE zzmdg_s_drf_pir_kf,
      lt_pir_data            TYPE zzmdg_t_drf_pir_kf.
...
*    INSERT MS_PIR_KEY_DATA INTO ls_pir_data.
      ...
      ls_pir_data-infnr = ms_pir_key_data-infnr.
