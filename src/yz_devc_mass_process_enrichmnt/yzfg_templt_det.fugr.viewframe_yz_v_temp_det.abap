*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_YZ_V_TEMP_DET
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_YZ_V_TEMP_DET       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_YZ_V_TEMP_DET'
         EXPORTING
              FCODE          = READ
...
EXTRACT = YZ_V_TEMP_DET_EXTRACT
TOTAL = YZ_V_TEMP_DET_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_YZ_V_TEMP_DET'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_YZ_V_TEMP_DET-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = YZ_V_TEMP_DET_EXTRACT
TOTAL = YZ_V_TEMP_DET_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_YZ_V_TEMP_DET-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_YZ_V_TEMP_DET'
...
TOTAL = YZ_V_TEMP_DET_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_YZ_V_TEMP_DET-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_YZ_V_TEMP_DET-UPD_FLAG NE SPACE.
